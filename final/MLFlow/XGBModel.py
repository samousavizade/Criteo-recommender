...
def check_existing_model():
    ...
    run_id = f.read()
    if run_id != '':
        MLFLOW_THREAD = threading.Thread(target=deploy, args=(run_id, MODEL_NAME))
        MLFLOW_THREAD.start()
...
def model():
    ...
    with mlflow.start_run() as run:
        ...
        # log optimal model
        mlflow.xgboost.log_model(optimal_model, MODEL_NAME)
        ...
        if MLFLOW_THREAD is not None:
            kill_thread = threading.Thread(target=kill_mlflow)
            kill_thread.start()
            kill_thread.join()
        MLFLOW_THREAD = threading.Thread(target=deploy, args=(run.info.run_id, MODEL_NAME))
        MLFLOW_THREAD.start()
    return ret_val
...
def deploy(run_id, model_name_in_mlflow):
    ...
    os.system(
        f'mlflow models serve -m "./mlruns/1/{run_id}/artifacts/{model_name_in_mlflow}" --no-conda '
